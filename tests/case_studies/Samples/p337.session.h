...
DATA_IN:		/* 1. prepare local file fsrver */
	...
	do
	:: ses_to_flow[1-n]?data,0 ->
		ses_to_fsrv[n]!data
	:: ses_to_flow[1-n]?eof,0 ->
		ses_to_fsrv[n]!eof;
		break
	:: pres_to_ses[n]?transfer ->
		ses_to_pres[n]!reject(NON_FATAL)
	:: ses_to_flow[1-n]?close,0 ->	/* remote user aborted */
		ses_to_fsrv[n]!close;
		break
...
DATA_OUT:		/* 1. prepare local file fsrver */
	...
	if
	:: ses_to_flow[1-n]?reject,0 ->
		ses_to_fsrv[n]!close;
		ses_to_pres[n]!reject(FATAL);
		goto endIDLE
	:: ses_to_flow[1-n]?connect,0 ->
		ses_to_fsrv[n]!close;
		ses_to_pres[n]!reject(NON_FATAL);
		goto endIDLE
	:: ses_to_flow[1-n]?accept,0 ->
		skip
	:: timeout ->
		ses_to_fsrv[n]!close;
		ses_to_pres[n]!reject(FATAL);
