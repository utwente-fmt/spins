...
progressDATA_IN:		/* 1. prepare local file fsrver */
	...
	do
	:: flow_to_ses[1-n]?data,0 ->
progress:	ses_to_fsrv[n]!data
	:: flow_to_ses[1-n]?eof,0 ->
		ses_to_fsrv[n]!eof;
		break
	:: pres_to_ses[n]?transfer ->
		ses_to_pres[n]!reject(NON_FATAL)
	:: flow_to_ses[1-n]?close,0 ->	/* remote user aborted */
		ses_to_fsrv[n]!close;
		break
...
progressDATA_OUT:		/* 1. prepare local file fsrver */
	...
	if
	:: flow_to_ses[1-n]?reject,status ->
		ses_to_fsrv[n]!close;
		ses_to_pres[n]!reject(FATAL);
		goto endIDLE
	:: flow_to_ses[1-n]?connect,0 ->
		ses_to_fsrv[n]!close;
		ses_to_pres[n]!reject(NON_FATAL);
		goto endIDLE
	:: flow_to_ses[1-n]?accept,0 ->
		skip
	:: timeout ->
		ses_to_fsrv[n]!close;
		ses_to_pres[n]!reject(FATAL);
