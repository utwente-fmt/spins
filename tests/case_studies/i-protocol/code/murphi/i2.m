...
PROCEDURE input(VAR Chan: Channel; VAR Hck, Dck: Checksum; VAR Pak: Packet);
BEGIN
    assert(isfull(Chan));
    Pak := Chan.body;
    Hck := Chan.hck;
...
RULESET n:ChanID DO
  ALIAS Chan: Channels[n] DO
	...
	isfull(Chan) & candrop & Chan.hck=1 & Chan.dck=1
    ==>
    BEGIN
	Chan.full := false;
	CLEAR Chan;
	...
	isfull(Chan) & cancorrupt=1 & Chan.hck=1
    ==>
    BEGIN
	Chan.hck := 0;
	observe(progress);
	...
	isfull(Chan) & cancorrupt=1 & Chan.dck=1
    ==>
    BEGIN
	Chan.dck := 0;
	observe(progress);
