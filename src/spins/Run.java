...
import spins.concurrent.model.ConcurrentModel;
import spins.concurrent.search.PartialOrderReduction;
import spins.exceptions.ValidationException;
import spins.options.BooleanOption;
import spins.options.MultiStringOption;
import spins.options.NumberOption;
import spins.options.OptionParser;
import spins.promela.model.NeverClaimModel;
import spins.promela.model.PromelaModel;
import spins.promela.model.PromelaTransition;
import spins.search.AcceptanceCycleSearch;
import spins.search.Algoritm;
import spins.search.BreadthFirstSearch;
import spins.search.ConcurrentBFSearch;
import spins.search.DepthFirstSearch;
import spins.search.RandomSimulation;
import spins.search.TrailSimulation;
import spins.search.TransitionCalculator;
import spins.search.UserSimulation;
import spins.store.AtomicHashTable;
import spins.store.BitstateHashStore;
import spins.store.HashCompactStore;
import spins.store.HashTable;
import spins.store.ProbingHashTable;
import spins.store.StateStore;
...
public class Run extends Thread {
	...
	public void search(final Class<? extends PromelaModel> clazz) {
		...
		if (impl.isSet("GUSER")) {
			algo = new UserSimulation<ConcurrentModel<PromelaTransition>, PromelaTransition>(model,
				nextTransition);
		} else if (impl.isSet("GTRAIL")) {
			try {
				algo = new TrailSimulation<ConcurrentModel<PromelaTransition>, PromelaTransition>(
			...
			algo = new RandomSimulation<ConcurrentModel<PromelaTransition>, PromelaTransition>(
				model, nextTransition);
			...
			algo = new ConcurrentBFSearch<ConcurrentModel<PromelaTransition>, PromelaTransition>(
				...
				nrErrors.getValue(), errorExceedDepth.isSet(), nextTransition);
